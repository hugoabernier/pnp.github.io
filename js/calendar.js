            ...
            if (thisMonday.getDay() !== 0 && thisMonday.getDay() !== 6) {
                ...
                for (let event of events) {
                    for (let occurrence of event.nextOccurrences) {
                        ...
                        occurrenceDate.setDate(occurrenceDate.getDate() + 1); // Add one day
                        let formattedDate = occurrenceDate.toISOString().split('T')[0];
                        let formattedTime = occurrenceDate.toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true });
